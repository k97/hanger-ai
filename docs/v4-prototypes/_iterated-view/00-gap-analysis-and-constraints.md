...
|---|---|---|
| **`tiktoken` for a static token footprint** of MCP schemas and skill files | No tokenizer dependency. `probe.rs` discards `inputSchema`; skill bodies are read whole (`read_asset_body`) | **Partial.** Bytes are a fact of the file; tokens are a fact of one tokenizer. Eleven engines tokenise differently, so "10.8k tokens" asserted without a tokenizer name is copy that is not literally true. Do bytes first (backend, medium: keep the schema size in `probe_results`); a tokeniser estimate can follow, labelled by the BPE it used (`tiktoken-rs` ships cl100k/o200k ranks, ~4 MB in the binary) |
