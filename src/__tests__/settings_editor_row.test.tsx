...
describe("EditorSetting", () => {
  it("marks the chosen editor as pressed and the others as not", () => {
    render(<EditorSetting editors={EDITORS} chosen="Zed" onChoose={vi.fn()} />);
  ...
  it("reports a change", () => {
    ...
    render(<EditorSetting editors={EDITORS} chosen="Zed" onChoose={onChoose} />);
    fireEvent.click(screen.getByRole("button", { name: "Cursor" }));
    expect(onChoose).toHaveBeenCalledWith("Cursor");
  });

  it("asserts no absence when nothing was detected", () => {
    render(<EditorSetting editors={[]} chosen={null} onChoose={vi.fn()} />);
