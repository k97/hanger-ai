...
const ALLOWLIST_RAW = [
  ...
  // the icon corner and uses the prototype's 9px numeral
  // (docs/v3/hanger-mono-tight.html .ir-count).
  { file: "src/components/IconRail.tsx", lineText: "<span aria-hidden=\"true\" className=\"absolute -top-[3px] -right-1 min-w-4 h-4 px-1 rounded-pill bg-fill text-on-fill text-[9px] leading-4 font-flex tabular\">", category: "fontsize" as AllowlistCategory, reason: "Icon-rail count badge, prototype 9px numeral inside a 16px pill" },
