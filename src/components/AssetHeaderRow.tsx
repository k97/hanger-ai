...
import { columnHeadClass } from "./typeRoles";
...
export default function AssetHeaderRow({
  ...
  const renderHeader = (field: SortField, label: string, widthClass: string) => {
    ...
    return (
      <button
        ...
        className={`flex items-center gap-1 cursor-pointer select-none ${columnHeadClass} hover:text-ink-1 transition-colors duration-hover focus:outline-none ${widthClass}`}
      >
        <span>{label}</span>
        {isActive && (
          sortDirection === "asc" ? (
  ...
  return (
    <div
      ...
      className="sticky top-0 z-[2] bg-page flex items-center gap-3 h-8 px-3.5 border-b border-line"
    >
      ...
      {showReachColumns ? (
        <>
          <span className={`hidden @[580px]:block w-[100px] shrink-0 text-left select-none ${columnHeadClass}`}>
            Reach
          </span>
          <span className={`w-[150px] shrink-0 text-left select-none truncate ${columnHeadClass}`}>
            Beyond the store
          </span>
        </>
      ) : (
